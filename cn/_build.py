...
# -*- coding: utf-8 -*-
"""组装《幽灵汇编》成品:
  1) 合并单一 Markdown 档  幽灵汇编_全书.md
  2) 图文 HTML(侧边子目录)  幽灵汇编.html
版面沿用系列标准 _build.py(同一套 CSS / 解析器),
内容为技术评注书(WebAssembly、静态托管与后护城河时代的工程实录)。
...
BOOK_TITLE = "幽灵汇编"
BOOK_SUB   = "WebAssembly、静态托管与后护城河时代的工程实录 · Ghost Assembly"
COVER_CANDIDATES = ["GhostAssembly.png", "幽灵汇编.png", "cover.png"]
...
def build_merged():
    ...
    parts.append("> 全书合并版。本书为教育与研究用途，横跨 WebAssembly 的运行机制、静态托管落地"
                 "与后护城河时代商业结构三大主题的整理与批判性评注。\n")
    parts.append("\n---\n\n## 目录\n")
    for base in FILES:
...
def md_to_html(text):
    ...
    while i < n:
        ...
        if ln.startswith(">"):
            ...
            if any("🔍" in b for b in blk):       # 进阶点评框(蓝绿)
                cls = "review"
