...
> **本書實測**：繁體約 **18 MB**／简体約 **18 MB**（各約 **340 頁**）／English 約 **16 MB**（約 **414 頁**）——**含整頁封面海報的書落在這個量級是正常的**。若某一版明顯偏離比例（例如破 100 MB），多半是手動列印時勾了「背景圖形」把 CSS 漸層逐頁點陣化，或多版平行列印互相汙染——依上列規則重印即可。
