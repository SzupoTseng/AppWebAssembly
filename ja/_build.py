#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Assemble the Japanese edition of Ghost Assembly:
...
BOOK_TITLE = "幽霊アセンブリ"
BOOK_SUB   = "存在しない機械と、あらゆるタブでの沈黙の実行 · Ghost Assembly"
...
def title_for(base, text):
    if base.startswith("00_"):
        m = re.search(r'^##\s+(まえがき.*)$', text, re.M)
        return m.group(1).strip() if m else "まえがき"
    m = re.search(r'^#\s+(第\s*\d+\s*章[：:].*)$', text, re.M)
    if m: return m.group(1).strip()
    m = re.search(r'^#\s+(付録\s*[A-Z][：:].*)$', text, re.M)
    return m.group(1).strip() if m else base

def part_for(text):
    m = re.search(r'^#\s+(第[一二三四五]部[：:].*)$', text, re.M)
...
def build_merged():
    ...
    cp = cover_path()
    if cp:
        parts.append(f"![{BOOK_TITLE} — カバーポスター]({os.path.basename(cp)})\n")
    parts.append("> 単一ファイル統合版。教育・研究用途：WebAssembly の実行モデル、"
                 "静的ホスティングへの配備、そしてポスト濠時代の商業構造を貫く技術評論。\n")
    parts.append("\n---\n\n## 目次\n")
    for base in FILES:
...
def cover_section():
    ...
    fig = (f'<section class="chapter cover-page" id="cover">'
           ...
           f'alt="{html.escape(BOOK_TITLE)} — カバーポスター"/></figure></section>')
