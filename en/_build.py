#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Assemble the English edition of Ghost Assembly:
  1) merged single Markdown  GhostAssembly_full.md
  2) illustrated HTML with sticky sidebar TOC  GhostAssembly.html
版面沿用系列標準 _build.py(同一套 CSS / 解析器),
內容為技術評註書(WebAssembly、靜態託管與後護城河時代的工程實錄)。
...
BOOK_TITLE = "Ghost Assembly"
BOOK_SUB   = "The machine that does not exist, and its silent execution in every tab"
OUTFILE = "GhostAssembly"
COVER_CANDIDATES = ["GhostAssembly.png", "cover.png"]
...
def title_for(base, text):
    ...
    m = re.search(r'^#\s+(Appendix\s+[A-Z][.:].*)$', text, re.M)
    return m.group(1).strip() if m else base

def part_for(text):
    m = re.search(r'^#\s+(Part\s+[IVX]+[.:].*)$', text, re.M)
...
def build_merged():
    ...
    parts.append("> Merged single-file edition. For educational and research use: a survey and "
                 "critical commentary spanning WebAssembly's execution model, its deployment on "
                 "static hosting, and the commercial structures of the post-moat era.\n")
    parts.append("\n---\n\n## Contents\n")
    for base in FILES:
...
if __name__ == "__main__":
    m = build_merged(); print("merged md:", m, f"({os.path.getsize(m)//1024} KB)")
